...
// English: A component to convert WebP images to JPG format, with controls for quality and background color.
...
const WebpToJpg: React.FC = () => {
  ...
  const form = useForm({
    ...
    validate: {
      files: (value) =>
        value.length === 0 ? 'Please upload at least one WebP file.' : null,
      prefix: (value) =>
        ...
        return new Promise<ProcessedFile | null>((resolve) => {
          ...
          reader.onload = (e) => {
            ...
            img.onload = () => {
              ...
              // English: Fill background for WebP transparency before drawing the image.
              ...
              canvas.toBlob(
                (blob) => {
                  if (blob) {
                    const newName = `${values.prefix}-${file.name.replace(/\.webp$/i, '')}.jpg`
                    resolve({
                      blob,
    ...
    <LayoutTool>
      <Stack>
        ...
        <Text c="dimmed" size="sm" ta="center" mb="lg">
          Convert your WebP images to the JPG format.
        </Text>
              ...
              <Stack>
                <Card withBorder p="xl" radius="md">
                  <Dropzone
                    ...
                    onReject={() =>
                      toast.error('Please upload only WebP images.')
                    }
                    accept={['image/webp']}
                  >
                    <Group
