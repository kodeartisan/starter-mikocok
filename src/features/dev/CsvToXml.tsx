// src/features/converter/CsvToXmlConverter.tsx
import LayoutTool from '@/components/Layout/LayoutTool'
import { onActionMellowtel } from '@/utils/util'
import { Icon } from '@iconify/react'
import {
  ActionIcon,
  Button,
  Card,
  Grid,
  Group,
  Stack,
  Switch,
  Text,
  Textarea,
  TextInput,
  Title,
  Tooltip,
} from '@mantine/core'
import { useForm } from '@mantine/form'
import { useClipboard } from '@mantine/hooks'
import { parse as toXml } from 'js2xmlparser'
import React, { useCallback, useState } from 'react'

const CsvToXml: React.FC = () => {
  const [output, setOutput] = useState('')
  const clipboard = useClipboard({ timeout: 1000 })
  const form = useForm({
    initialValues: {
      input: '',
      hasHeader: true,
      rootName: 'root',
      rowName: 'row',
    },
  })

  const handleSubmit = useCallback((values: typeof form.values) => {
    onActionMellowtel(() => {
      if (!values.input) {
        setOutput('')
        return
      }
      try {
        const lines = values.input.trim().split(/\r?\n/)
        if (lines.length === 0) {
          setOutput(`<${values.rootName}></${values.rootName}>`)
          return
        }

        const headers = values.hasHeader
          ? lines.shift()!.split(',')
          : lines[0].split(',').map((_, i) => `field${i + 1}`)

        const jsonArray = lines.map((line) => {
          const values = line.split(',')
          return headers.reduce((obj, header, index) => {
            obj[header.trim().replace(/\s/g, '_')] = values[index]?.trim() // Sanitize header for XML tag
            return obj
          }, {})
        })

        const dataForXml = { [values.rowName]: jsonArray }
        const xml = toXml(values.rootName, dataForXml, {
          declaration: { include: true },
          format: { doubleQuotes: true, pretty: true },
        })
        setOutput(xml)
      } catch (error: any) {
        setOutput(`Error: ${error.message}`)
      }
    })
  }, [])

  const handleClear = useCallback(() => {
    form.reset()
    setOutput('')
  }, [form])

  return (
    <LayoutTool width={1000}>
      <Stack>
        <Title order={4} ta="center">
          CSV to XML Converter
        </Title>
        <Text c="dimmed" size="sm" ta="center" mb="lg">
          Convert your CSV data into a well-structured XML format.
        </Text>
        <form onSubmit={form.onSubmit(handleSubmit)}>
          <Stack>
            <Card withBorder>
              <Group grow>
                <Switch
                  label="First row is header"
                  {...form.getInputProps('hasHeader', { type: 'checkbox' })}
                />
                <TextInput
                  label="Root Element Name"
                  {...form.getInputProps('rootName')}
                />
                <TextInput
                  label="Row Element Name"
                  {...form.getInputProps('rowName')}
                />
              </Group>
            </Card>
            <Grid>
              <Grid.Col span={{ base: 12, md: 6 }}>
                <Textarea
                  label="Input CSV"
                  placeholder="header1,header2&#10;value1,value2"
                  autosize
                  minRows={12}
                  styles={{ input: { fontFamily: 'monospace' } }}
                  {...form.getInputProps('input')}
                />
              </Grid.Col>
              <Grid.Col span={{ base: 12, md: 6 }}>
                <Textarea
                  label="Output XML"
                  readOnly
                  autosize
                  minRows={12}
                  value={output}
                  styles={{ input: { fontFamily: 'monospace' } }}
                />
              </Grid.Col>
            </Grid>
            <Card withBorder>
              <Group justify="space-between">
                <Button type="submit">Convert</Button>
                <Group>
                  <Tooltip label="Copy">
                    <ActionIcon
                      variant="light"
                      size="lg"
                      onClick={() => clipboard.copy(output)}
                      disabled={!output}
                    >
                      <Icon
                        icon={clipboard.copied ? 'tabler:check' : 'tabler:copy'}
                      />
                    </ActionIcon>
                  </Tooltip>
                  <Tooltip label="Clear">
                    <ActionIcon
                      variant="light"
                      color="red"
                      size="lg"
                      onClick={handleClear}
                    >
                      <Icon icon="tabler:clear-all" />
                    </ActionIcon>
                  </Tooltip>
                </Group>
              </Group>
            </Card>
          </Stack>
        </form>
      </Stack>
    </LayoutTool>
  )
}

export default CsvToXml
