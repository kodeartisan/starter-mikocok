// src/features/Generator/JsMinifier.tsx
...
const JsMinifier: React.FC = () => {
  ...
  const handleSubmit = async (values: { input: string }) => {
    await onActionMellowtel(async () => {
      ...
      setIsLoading(true)
      try {
        const result = await minify(values.input, {
          mangle: true,
          compress: true,
        })
        if (result.code) {
    ...
    <LayoutTool width={1000}>
      <Stack>
        ...
        <Text c="dimmed" size="sm" ta="center" mb="lg">
          Obfuscate and compress your JavaScript code to significantly reduce
          its size and protect your source.
        </Text>
