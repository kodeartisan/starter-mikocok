// src/features/Generator/HtmlMinifier.tsx
  ...
  const handleSubmit = async (values: { input: string }) => {
    await onActionMellowtel(async () => {
      ...
      try {
        ...
        const result = await minify(values.input, options)
        setOutput(result)
      } catch (error: any) {
    ...
    <LayoutTool width={1000}>
      <Stack>
        ...
        <Text c="dimmed" size="sm" ta="center" mb="lg">
          Reduce the size of your HTML files by removing unnecessary characters
          and whitespace.
        </Text>
